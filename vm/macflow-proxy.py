"""MACFlow proxy: Windows :18080 -> WSL curl -> QEMU :18080 -> iStoreOS"""
import http.server
import subprocess
import sys
import tempfile
...
class Proxy(http.server.BaseHTTPRequestHandler):
    def _do(self):
        ...
        if sep == -1:
            ...
            ct = "text/html" if self.path == "/" else "application/json"
...
if __name__ == "__main__":
    print(f"MACFlow proxy: http://localhost:{PORT}")
    print(f"Target: {TARGET} (via WSL -> QEMU -> iStoreOS)")
